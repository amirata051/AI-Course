# Minimax.py with Alpha Beta Pruning
...
class Minimax:
    ...
    def minimax_move(self):
        ...
        for move in possible_moves:
            ...
            utility = self.min_value(pacman_copy, self.ghosts, depth=3, alpha=float('-inf'), beta=float('inf'))
    ...
    def max_value(self, pacman, ghosts, depth, alpha, beta):
        ...
        for move in ['up', 'down', 'left', 'right']:
            ...
            utility = self.min_value(pacman_copy, ghosts, depth - 1, alpha, beta)
    ...
    def min_value(self, pacman, ghosts, depth, alpha, beta):
        if depth == 0 or pacman.end_game(ghosts):
            return self.evaluate_state(pacman, ghosts)
        ...
        for ghost in ghosts:
            ...
            utility = self.max_value(pacman, [ghost_copy], depth - 1, alpha, beta)
    ...
    def evaluate_state(self, pacman, ghosts):
        # Criteria:
        # 1. Pacman's score
        # 2. Relative positions of Pacman and ghosts
        ...
        position_utility = 0
        for ghost in ghosts:
            position_utility += self.calculate_relative_position_utility(pacman, ghost)
    ...
    def calculate_nearest_ghost_utility(self, pacman, ghosts):
        ...
        for ghost in ghosts:
            distance = abs(pacman.get_position()[0] - ghost.get_position()[0]) + abs(
                pacman.get_position()[1] - ghost.get_position()[1]
            )
            nearest_ghost_distance = min(nearest_ghost_distance, distance)

        return -500 / (nearest_ghost_distance + 1)  # Adjust the penalty as needed
    ...
    def calculate_distance_between_pacma_and_ghosts_utility(self, pacman, ghosts):
        ...
        return 500 * distance  # Adjust the reward as needed